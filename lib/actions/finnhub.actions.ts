...
async function fetchJSON<T = any>(url: string, revalidateSeconds?: number): Promise<T> {
...
export async function getNews(symbols?: string[]) {
    try {
            ...
            await Promise.all(
                cleanSymbols.map(async (sym) => {
                    try {
                        ...
                        const articles = await fetchJSON(url, 300) as any[];
        ...
        const general = await fetchJSON(generalUrl, 300) as any[];
